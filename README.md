...
-   **HTMLエクスポート**: フィルタリングで絞り込んだ現在の表示結果を、背景色や文字色のスタイルを維持したまま、新しいHTMLファイルとして書き出すことができます。ファイル名やページタイトルもカスタマイズ可能です。
...
-   **高度なダイスロールフィルタ (CoC6版)**:
    -   クトゥルフ神話TRPG第6版の多様なダイスロール結果を詳細に分析し、特定の条件で抽出できます。
    -   **抽出対象**:
        -   全てのダイスロール結果（成否判定のないものも含む）
        -   成功 / 失敗
        -   **スペシャル**: 目標値の1/5以下の出目（クリティカルを除く）
        -   **決定的成功（クリティカル）**: 出目が1〜5
        -   **致命的失敗（ファンブル）**: 出目が96〜100
    -   **対応コマンド**:
        -   `CCB`, `1d100` (通常のスキルチェック)
        -   `CBRB` (コンバインロール)
        -   `RESB` (対抗ロール / 自動成功・失敗にも対応)
        -   `xN ... #M ...` (複数回ロール)
        -   `sCOMMAND` (シークレットダイス / `s CCB`と`sCCB`の両方に対応)
        -   `2d6`や`1d3`などの一般的なダイスロール
        -   `choice` コマンド

### その他の機能
-   **タブUI**: 「CoC6版」と「CoC7版」の機能を切り替えるためのタブインターフェースを備えています。（現在、CoC7版は準備中です）
-   **プライバシーへの配慮**:
    -   検索エンジンに登録されないよう`noindex`メタタグを設定済みです。
    -   エクスポート時に、シークレットダイスの内容を「シークレットダイス」という文字に置き換えて秘匿するオプションがあります。
...
6.  必要に応じて、以下の操作を行います。
    ...
    -   **出力**:
        -   出力するHTMLのタイトルを任意で入力します。
        -   シークレットダイスの内容を隠したい場合はチェックを入れます。
        -   [フィルタ結果をHTMLに出力]ボタンをクリックすると、現在の表示内容がHTMLファイルとして保存されます。
